...
%type <hilti::ExpressionPtr>                expr tuple_elem tuple_expr member_expr ctor_expr expr_0 expr_1 expr_2 expr_3 expr_4 expr_5 expr_6 expr_7 expr_8 expr_9 expr_a expr_b expr_c expr_d expr_e expr_f expr_g opt_init_expression opt_unit_field_condition unit_field_repeat opt_unit_field_repeat opt_unit_switch_expr
...
%type <spicy::HookPtr>                       unit_hook
%type <spicy::Hooks>                         opt_unit_item_hooks unit_hooks
...
module        : MODULE local_id ';'              { _docs.emplace_back(driver->docGetAndClear()); }
                global_scope_items               { auto uid = hilti::module::UID($2, hilti::rt::filesystem::path(*driver->currentFile()));
...
property_decl : PROPERTY ';'                     { $$ = builder->declarationProperty(ID(std::move($1)), __loc__); }
              ...
              | PROPERTY '=' qtype ';'       { $$ = builder->declarationProperty(ID(std::move($1)), builder->expressionType(std::move($3)), __loc__); }
              ;
...
qtype         : type                             { $$ = builder->qualifiedType(std::move($1), false, __loc__); }
              ...
              | AUTO                             { $$ = builder->qualifiedType(builder->typeAuto(__loc__), true, __loc__); }
              ;
...
unit_hooks    : unit_hooks unit_hook             { $$ = std::move($1); $$.push_back(std::move($2)); }
              | unit_hook                        { $$ = spicy::Hooks{std::move($1)}; }

unit_hook     : opt_unit_hook_params opt_unit_hook_attributes opt_hook_engine braced_block
                                                 { $$ = builder->hook(std::move($1), std::move($4), std::move($3), std::move($2), __loc__); }
