...
struct VisitorPost : visitor::PreOrder, hilti::validator::VisitorMixIn {
    ...
    void checkVariable(const GlobalOrLocalVariable& n) {
        ...
        const bool isTyped = n->type()->type()->typeID().has_value();
        if ( isTyped )
            return;
        ...
        if ( auto init = n->init() ) {
            if ( auto ctor = init->template tryAs<hilti::expression::Ctor>() ) {
                if ( ctor->ctor()->template tryAs<hilti::ctor::Struct>() ) {
                    error("declaration needs a concrete struct type", n);
                }
            }
        }
    }
    ...
    void operator()(hilti::expression::Name* n) final {
        if ( n->id() == ID("__dd") ) {
            if ( auto hook = n->parent<spicy::Hook>(); hook && hook->isForEach() )
                // $$ in "foreach" ok is ok.
                return;
    ...
    void operator()(statement::Stop* n) final {
        ...
        if ( auto x = n->parent<Hook>(); ! (x && x->isForEach()) )
    ...
    void operator()(spicy::Hook* n) final {
        if ( auto field = n->parent<spicy::type::unit::item::Field>() ) {
    ...
    void operator()(spicy::type::unit::item::UnitHook* n) final {
        ...
        _checkHook(unit, n->hook(), decl->linkage() == hilti::declaration::Linkage::Public, false, n);
    }

    void operator()(hilti::Attribute* n) final {
        auto builder = Builder(VisitorMixIn::builder());

        auto getAttrField = [](hilti::Attribute* a) -> spicy::type::unit::item::FieldPtr {
            try {
                ...
                if ( a->tag() == "&size" || a->tag() == "&max-size" ) {
                    if ( ! a->hasValue() )
                        ...
                        for ( auto i : v.walk(a->value()) )
                            if ( const auto& id = i->tryAs<ID>(); id && id->str() == "self" ) {
    ...
    void operator()(spicy::type::unit::item::Field* n) final {
        auto count_attr = n->attributes()->find("&count");
        auto repeat = n->repeatCount();
        auto is_sub_item = n->parent()->isA<spicy::type::unit::item::Field>();
        ...
        if ( const auto& c = n->ctor() ) {
            ...
            if ( n->originalType()->type()->isA<hilti::type::Vector>() && is_sub_item ) {
                error("use [] syntax to parse vectors", n);
                return;
            }
    ...
    void operator()(spicy::declaration::UnitHook* n) final {
        if ( const auto& ut = n->hook()->unitType() )
            _checkHook(ut.get(), n->hook(), ut->isPublic(), true, n);
        else
            error("unknown unit type", n);
    }

    void _checkHook(const type::Unit* unit, const HookPtr& hook, bool is_public, bool is_external, Node* n) {
...
void detail::validator::validate_pre(Builder* builder, const ASTRootPtr& root) {
    ...
    visitor::visit(VisitorPre(builder), root);
    (*hilti::plugin::registry().hiltiPlugin().ast_validate_pre)(builder->context(), root);
}

void detail::validator::validate_post(Builder* builder, const ASTRootPtr& root) {
    hilti::util::timing::Collector _("spicy/compiler/ast/validator");
    visitor::visit(VisitorPost(builder), root);
    (*hilti::plugin::registry().hiltiPlugin().ast_validate_post)(builder->context(), root);
}
