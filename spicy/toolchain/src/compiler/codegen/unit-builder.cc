...
UnqualifiedTypePtr CodeGen::compileUnit(const type::UnitPtr& unit, bool declare_only) {
    ...
    auto attr_sync =
        builder()->attributeSet({builder()->attribute("&needed-by-feature", builder()->string("synchronization"))});
    add_hook("0x25_confirmed", {}, attr_sync);
    add_hook("0x25_rejected", {}, attr_sync);
    add_hook("0x25_synced", {}, attr_sync);

    if ( auto id = unit->typeID() ) {
        ID typeID = ID(hilti::rt::replace(*id, ":", "@"));
        ...
        addDeclaration(
            builder()->constant(ID(fmt("__feat%%%s%%is_filter", typeID)), builder()->bool_(unit->isFilter())));
    ...
    auto attr_uses_random_access = builder()->attribute("&needed-by-feature", builder()->string("uses_random_access"));
    auto iter = builder()->qualifiedType(builder()->typeStreamIterator(), hilti::Constness::NonConst);
    auto f1 =
        builder()->declarationField(ID("__begin"),
    ...
    auto attr_uses_offset = builder()->attribute("&needed-by-feature", builder()->string("uses_offset"));
    auto f3 =
...

    {
        auto attrs = builder()->attributeSet(
            {builder()->attribute("&static"), builder()->attribute("&internal"),
             builder()->attribute("&needed-by-feature", builder()->string("supports_filters"))});

        if ( unit->isPublic() )
            attrs->add(context(), builder()->attribute("&always-emit"));
        else
            attrs->add(context(), builder()->attribute("&needed-by-feature", builder()->string("supports_sinks")));

        if ( unit->isFilter() )
            attrs->add(context(), builder()->attribute("&needed-by-feature", builder()->string("is_filter")));
        ...
        auto forward =
            builder()
                ->declarationField(ID("__forward"),
                                   ...
                                   builder()->attributeSet(
                                       {builder()->attribute("&internal"),
                                        builder()->attribute("&needed-by-feature", builder()->string("is_filter"))}));
    ...
    // s->setTypeID(*unit->typeID()); TODO: Needed?
    ...
    _pb.guardFeatureCode(*unit->typeID(), dependentFeatureFlags, [&]() {
        ...
        auto parser = builder()->struct_(
            {builder()->ctorStructField(ID("name"), builder()->string(*unit->typeID())),
             ...
             builder()->ctorStructField(ID("description"),
                                        (description ? description->expression() : builder()->string(""))),
        ...
        _pb.builder()->addExpression(
            builder()->call("spicy_rt::registerParser",
                            {builder()->id(ID(*unit->typeID(), "__parser")), builder()->scope(),
                             builder()->strong_reference(builder()->qualifiedType(unit, hilti::Constness::Const))}));
    });

    auto block = _pb.popBuilder()->block();

    auto register_unit = builder()->function(ID(fmt("__register_%s_%s", hiltiUnit()->uid(), unit->typeID()->local())),
