...
struct Visitor : public visitor::PreOrder {
    ...
    std::unique_ptr<Production> productionForType(const QualifiedTypePtr& t, const ID& id) {
        if ( auto prod = pf->createProduction(t) )
            return prod;
        else
    ...
    void operator()(spicy::type::unit::item::Field* n) final {
        if ( n->isSkip() ) {
            ...
            if ( const auto& ctor = n->ctor() ) {
                ...
                auto size_attr = n->attributes()->find("&size");
    ...
    void operator()(type::Unit* n) final {
        ...
        if ( auto p = pf->cache.get(id) ) {
            auto r = dynamic_cast<production::Resolved*>(*p);
            assert(r && r->resolved());
            result = std::make_unique<production::Reference>(context(), r->resolved());
            return;
        }

        // Prime the cache for any self-recursive unit productions.
        auto unresolved = std::make_unique<production::Resolved>(context());
        ...
        pf->grammar->resolve(dynamic_cast<production::Resolved*>(unresolved.get()), std::move(unit));
...
hilti::Result<hilti::Nothing> GrammarBuilder::run(const hilti::NodeDerivedPtr<type::Unit>& unit) {
    assert(unit->typeID());
    auto id = *unit->typeID();
