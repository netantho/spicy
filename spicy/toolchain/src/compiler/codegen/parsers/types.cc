...
struct TypeParser {
    ...
    ExpressionPtr performUnpack(const ExpressionPtr& target, const UnqualifiedTypePtr& t, int len,
...
struct Visitor : public visitor::PreOrder {
    ...
    void operator()(hilti::type::SignedInteger* n) final {
        result = tp->performUnpack(tp->destination(n->as<hilti::type::Real>()), n->as<hilti::type::Real>(),
                                   static_cast<int>(n->width() / 8), {state().cur, tp->fieldByteOrder()}, n->meta(),
                                   tp->is_try);
    }
    ...
    void operator()(hilti::type::Bytes* n) final {
        ...
        bool to_eod = eod_attr->hasValue(); // parse to end of input data
