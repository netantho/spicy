...
void production::While::preprocessLookAhead(ASTContext* ctx, Grammar* grammar) {
    ...
    auto unresolved = std::make_unique<production::Resolved>(ctx);
    ...
    l2_prods.emplace_back(
        std::move(body_ref)); // TODO: Can't get direct initialize of the vector to work for some reason?
