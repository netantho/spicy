...
public:
    ...
    auto builder() const { return _builders.back().get(); }
    ...
    NodeDerivedPtr<hilti::type::Function> parseMethodFunctionType(
     ...
     */
    void waitForInput(const ExpressionPtr& min, const std::string& error_msg, const Meta& location);
     ...
     */
    void consumeLookAhead(std::optional<ExpressionPtr> dst = {});

    /** Generates code that triggers a parse error exception. */
    void parseError(const std::string& error_msg, const Meta& location);

    /** Generates code that triggers a parse error exception. */
    void parseError(const ExpressionPtr& error_msg, const Meta& location);

    /** Generates code that triggers a parse error exception. */
    void parseError(const std::string& fmt, const Expressions& args, const Meta& location);
    ...
    hilti::ExpressionPtr featureConstant(const hilti::ID& typeID, std::string_view feature);
