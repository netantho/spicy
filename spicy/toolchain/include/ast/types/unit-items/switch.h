...
public:
    ...
    static auto create(ASTContext* ctx, ExpressionPtr expr, type::unit::item::switch_::Cases cases, Engine engine,
                       ExpressionPtr cond, spicy::Hooks hooks, AttributeSetPtr attrs, const Meta& meta = {}) {
        if ( ! attrs )
            attrs = AttributeSet::create(ctx);

        return NodeDerivedPtr<Switch>(
            new Switch(ctx,
...
protected:
    Switch(ASTContext* ctx, Nodes children, Engine engine, const Meta& meta)
        : unit::Item(ctx, std::move(children), meta), _engine(engine) {}
