...
public:
    ...
    static auto create(ASTContext* ctx, ctor::unit::Fields fields, const Meta& meta = {}) {
        ...
        return NodeDerivedPtr<Unit>(new Unit(ctx, node::flatten(std::move(auto_), std::move(fields)), meta));
    }

    static auto create(ASTContext* ctx, ctor::unit::Fields fields, QualifiedTypePtr t, const Meta& meta = {}) {
        return NodeDerivedPtr<Unit>(new Unit(ctx, node::flatten(std::move(t), std::move(fields)), meta));
    }
