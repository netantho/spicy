...
public:
    ...
    static auto create(ASTContext* ctx, const UnqualifiedTypePtr& base, Meta meta = {}) {
        return NodeDerivedPtr<Exception>(new Exception(ctx, {base}, std::move(meta)));
    }

    static auto create(ASTContext* ctx, const Meta& meta = {}) { return create(ctx, nullptr, meta); }

    static auto create(ASTContext* ctx, Wildcard _, const Meta& m = Meta()) {
        return NodeDerivedPtr<Exception>(new Exception(ctx, Wildcard(), {type::Unknown::create(ctx, m)}, m));
    }
