...
public:
    ...
    static auto create(ASTContext* ctx, ID id, const QualifiedTypePtr& type, Meta meta = {}) {
        return NodeDerivedPtr<Element>(new Element(ctx, {type}, std::move(id), std::move(meta)));
    }

    static auto create(ASTContext* ctx, const QualifiedTypePtr& type, Meta meta = {}) {
        return NodeDerivedPtr<Element>(new Element(ctx, {type}, ID(), std::move(meta)));
    }
...
public:
    ...
    static auto create(ASTContext* ctx, type::tuple::Elements elements, Meta meta = {}) {
        return NodeDerivedPtr<Tuple>(new Tuple(ctx, std::move(elements), std::move(meta)));
    }
    ...
    static auto create(ASTContext* ctx, const QualifiedTypes& types, const Meta& meta = {}) {
        ...
        return NodeDerivedPtr<Tuple>(new Tuple(ctx, std::move(elements), meta));
    }

    static auto create(ASTContext* ctx, Wildcard _, const Meta& m = Meta()) {
        return NodeDerivedPtr<Tuple>(new Tuple(ctx, Wildcard(), m));
    }
