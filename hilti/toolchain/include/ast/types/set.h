...
public:
    ...
    static auto create(ASTContext* ctx, const QualifiedTypePtr& t, const Meta& meta = {}) {
        return NodeDerivedPtr<Set>(
            new Set(ctx, {QualifiedType::create(ctx, set::Iterator::create(ctx, t, meta), false)}, meta));
    }

    static auto create(ASTContext* ctx, Wildcard _, const Meta& m = Meta()) {
        return NodeDerivedPtr<Set>(
            new Set(ctx, Wildcard(), {QualifiedType::create(ctx, set::Iterator::create(ctx, Wildcard(), m), false)},
                    m));
    }
