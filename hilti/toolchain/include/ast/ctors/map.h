...
public:
    ...
    static auto create(ASTContext* ctx, const ExpressionPtr& key, const ExpressionPtr& value, Meta meta = {}) {
        return NodeDerivedPtr<Element>(new Element(ctx, {key, value}, std::move(meta)));
    }
...
public:
    ...
    static auto create(ASTContext* ctx, const QualifiedTypePtr& key, const QualifiedTypePtr& value,
        ...
        auto mtype = QualifiedType::create(ctx, type::Map::create(ctx, key, value, meta), false, meta);
        return NodeDerivedPtr<Map>(new Map(ctx, node::flatten(mtype, std::move(elements)), meta));
    }
    ...
    static auto create(ASTContext* ctx, map::Elements elements, const Meta& meta = {}) {
        ...
        auto mtype = elements.empty() ? QualifiedType::create(ctx, type::Bool::create(ctx, meta), false, meta) :
