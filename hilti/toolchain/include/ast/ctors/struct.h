...
public:
    ...
    static auto create(ASTContext* ctx, ID id, const ExpressionPtr& expr, Meta meta = {}) {
        return NodeDerivedPtr<Field>(new Field(ctx, {expr}, std::move(id), std::move(meta)));
    }
...
public:
    ...
    static auto create(ASTContext* ctx, struct_::Fields fields, QualifiedTypePtr t, const Meta& meta = {}) {
        return NodeDerivedPtr<Struct>(new Struct(ctx, node::flatten(std::move(t), std::move(fields)), meta));
    }
