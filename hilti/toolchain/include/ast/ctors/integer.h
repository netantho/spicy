...
public:
    static auto create(ASTContext* ctx, int64_t value, unsigned int width, const Meta& meta = {}) {
        return CtorPtr(
            new SignedInteger(ctx, {QualifiedType::create(ctx, type::SignedInteger::create(ctx, width, meta), true)},
                              value, width, meta));
    }
...
public:
    static auto create(ASTContext* ctx, uint64_t value, unsigned int width, const Meta& meta = {}) {
        return CtorPtr(
            new UnsignedInteger(ctx,
                                {QualifiedType::create(ctx, type::UnsignedInteger::create(ctx, width, meta), true)},
                                value, width, meta));
    }

    static auto create(ASTContext* ctx, uint64_t value, unsigned int width, const UnqualifiedTypePtr& t,
                       const Meta& meta = {}) {
        return CtorPtr(new UnsignedInteger(ctx, {QualifiedType::create(ctx, t, true)}, value, width, meta));
    }
