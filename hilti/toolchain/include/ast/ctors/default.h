...
public:
    ...
    static auto create(ASTContext* ctx, const UnqualifiedTypePtr& type, const Meta& meta = {}) {
        return NodeDerivedPtr<Default>(new Default(ctx, {QualifiedType::create(ctx, type, true, meta)}, meta));
    }
    ...
    static auto create(ASTContext* ctx, const UnqualifiedTypePtr& type, Expressions type_args, const Meta& meta = {}) {
        return CtorPtr(
            new Default(ctx, node::flatten(QualifiedType::create(ctx, type, true, meta), std::move(type_args)), meta));
    }
