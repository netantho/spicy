...
public:
    ...
    static auto create(ASTContext* ctx, const QualifiedTypePtr& etype, Expressions exprs, const Meta& meta = {}) {
        auto stype = QualifiedType::create(ctx, type::Vector::create(ctx, etype, meta), false, meta);
        return NodeDerivedPtr<Vector>(new Vector(ctx, node::flatten(stype, std::move(exprs)), meta));
    }
    ...
    static auto create(ASTContext* ctx, Expressions exprs, const Meta& meta = {}) {
        ...
        auto etype = exprs.empty() ? QualifiedType::create(ctx, type::Bool::create(ctx, meta), false, meta) :
