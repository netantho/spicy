...
public:
    ...
    static auto create(ASTContext* ctx, std::string value, const Meta& meta = {}) {
        return CtorPtr(new String(ctx, {QualifiedType::create(ctx, type::String::create(ctx, meta), true)},
                                  std::move(value), meta));
    }

protected:
    String(ASTContext* ctx, Nodes children, std::string value, Meta meta)
