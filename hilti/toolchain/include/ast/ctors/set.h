...
public:
    ...
    static auto create(ASTContext* ctx, const QualifiedTypePtr& etype, Expressions exprs, const Meta& meta = {}) {
        auto stype = QualifiedType::create(ctx, type::Set::create(ctx, etype, meta), false, meta);
        return NodeDerivedPtr<Set>(new Set(ctx, node::flatten(stype, std::move(exprs)), meta));
    }
