...
public:
    ...
    static auto create(ASTContext* ctx, const ExpressionPtr& input, const ExpressionPtr& output, const ID& id,
        ...
        auto list =
            QualifiedType::create(ctx, type::List::create(ctx, QualifiedType::createAuto(ctx, meta), meta), true);
        return NodeDerivedPtr<ListComprehension>(new ListComprehension(ctx, {input, output, local, cond, list}, meta));
    }
