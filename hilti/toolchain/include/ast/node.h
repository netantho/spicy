 ...
 */
template<typename T>
NodeDerivedPtr<T> deepcopy(ASTContext* ctx, const NodeDerivedPtr<T>& n, bool force = false) {
    if ( ! n )
        return nullptr;
...
protected:
    ...
    /**
     * Returns additional information to include into the node's `render()`
...
private:
    ...
    void _checkThisForCast() const {
        ...
        // easier.
        if ( std::is_base_of<UnqualifiedType, T>::value && ! std::is_same<UnqualifiedType, T>::value )
