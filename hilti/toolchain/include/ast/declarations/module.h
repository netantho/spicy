...
struct UID {
    ...
    bool operator<(const UID& other) const { return unique < other.unique; }
...
public:
    ...
    static auto create(ASTContext* ctx, const module::UID& uid, const ID& scope, const Declarations& decls,
        ...
        return NodeDerivedPtr<Module>(new Module(ctx, std::move(nodes), uid, scope, meta));
    }
...
protected:
    Module(ASTContext* ctx, Nodes children, module::UID uid, ID scope, Meta meta = {})
...
private:
    ...
    bool _skip_implementation = false;
};
