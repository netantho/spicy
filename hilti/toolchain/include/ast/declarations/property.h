...
public:
    ...
    static auto create(ASTContext* ctx, ID id, Meta meta = {}) {
        return NodeDerivedPtr<Property>(new Property(ctx, {}, std::move(id), std::move(meta)));
    }

    static auto create(ASTContext* ctx, ID id, const ExpressionPtr& expr, Meta meta = {}) {
        return NodeDerivedPtr<Property>(new Property(ctx, {expr}, std::move(id), std::move(meta)));
    }
