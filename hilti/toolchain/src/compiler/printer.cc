...
detail::printer::Stream& detail::printer::Stream::operator<<(const ID& id) {
...
struct Printer : visitor::PreOrder {
    Printer(detail::printer::Stream& out) : out(out) {} // NOLINT
    ...
    auto const_(const QualifiedType* t) {
        return (out.isCompact() && t->isConstant() && t->type()->isMutable()) ? "const " : "";
    }
    ...
    void operator()(ctor::Address* n) final { out << n->value(); }
    ...
    void operator()(type::Bitfield* n) final {
        ...
        out << fmt("bitfield(%d) {", n->width()) << out.newline();
    ...
    void operator()(type::OperandList* n) final { out << "(" << std::make_pair(n->operands(), ", ") << ")"; }
    ...
    void operator()(type::Struct* n) final {
        ...
        out << " {" << out.newline();
        printFields(util::filter(n->fields(),
...
private:
    detail::printer::Stream& out;
};

} // anonymous namespace

void detail::printer::print(std::ostream& out, const NodePtr& root, bool compact) {
    if ( ! State::_current )
        State::_current = std::make_unique<State>();
...
void detail::printer::print(printer::Stream& stream, const NodePtr& root) {
