...
#include <hilti/autogen/ast-operators.h>
...
static Result<CtorPtr> _foldConstant(Builder* builder, const ExpressionPtr& expr);

template<typename Ctor>
Result<Ctor> foldConstant(Builder* builder, const ExpressionPtr& expr) {
    auto ctor = _foldConstant(builder, expr);
    if ( ! ctor )
        return ctor.error();
...
struct VisitorConstantFolder : public visitor::PreOrder {
    ...
    void operator()(expression::Grouping* n) final {
        if ( auto x = _foldConstant(builder, n->expression()) )
            result = *x;
    }
    ...
    void operator()(expression::Name* n) final {
        ...
        auto const_ = n->resolvedDeclaration()->tryAs<declaration::Constant>();
        if ( ! const_ )
            return;

        auto x = _foldConstant(builder, const_->value());
        if ( ! x )
            return;
...
Result<CtorPtr> _foldConstant(Builder* builder, const ExpressionPtr& expr) {
    if ( auto result =
