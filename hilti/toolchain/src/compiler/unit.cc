...
std::shared_ptr<Unit> Unit::fromExistingUID(const std::shared_ptr<Context>& context, module::UID uid) {
...
Result<detail::cxx::Unit> Unit::_codegenModule(const module::UID& uid) {
    auto module = context()->astContext()->getModule(uid);
    assert(module);

    HILTI_DEBUG(logging::debug::Compiler, fmt("compiling module %s to C++", uid));
    logging::DebugPushIndent _(logging::debug::Compiler);

    auto cxx = detail::CodeGen(context()).compileModule(module, this, ! module->skipImplementation());
