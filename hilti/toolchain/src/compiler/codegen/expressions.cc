...
struct Visitor : hilti::visitor::PreOrder {
    ...
    void operator()(expression::Name* n) final {
        ...
        auto id = n->resolvedDeclaration()->fullyQualifiedID();
        assert(id);

        if ( auto g = n->resolvedDeclaration()->tryAs<declaration::GlobalVariable>() ) {
            if ( cg->options().cxx_enable_dynamic_globals ) {
                if ( auto ns = id.namespace_(); ! ns.empty() )
                    result = {fmt("%s->%s", cxx::ID(ns, "__globals()"), cxx::ID(id.local())), Side::LHS};
                else
                    result = {fmt("__globals()->%s", cxx::ID(id)), Side::LHS};
            }
            else
                result = {fmt("(*%s)", cxx::ID(cg->options().cxx_namespace_intern, cxx::ID(id))), Side::LHS};

            return;
        }

        if ( auto e = n->resolvedDeclaration()->tryAs<declaration::Expression>() ) {
            result = cg->compile(e->expression(), lhs);
            return;
        }

        if ( auto c = n->resolvedDeclaration()->tryAs<declaration::Constant>() ) {
            if ( c->value()->type()->type()->isA<type::Enum>() )
                result = {cxx::ID(cg->compile(c->value())), Side::LHS};
            else
                result = {cxx::ID(cg->options().cxx_namespace_intern, cxx::ID(id)), Side::LHS};

            return;
        }

        if ( auto f = n->resolvedDeclaration()->tryAs<declaration::Function>() ) {
            ...
            if ( (f->function()->callingConvention() == function::CallingConvention::Extern ||
                ...
                if ( id.namespace_().empty() )
                    ...
                    result = {cxx::ID(id), Side::LHS};
                else
                    result = {cxx::ID(cg->options().cxx_namespace_extern, id), Side::LHS};

                return;
            }
        }

        if ( auto p = n->resolvedDeclaration()->tryAs<declaration::Parameter>(); p && p->isTypeParameter() ) {
