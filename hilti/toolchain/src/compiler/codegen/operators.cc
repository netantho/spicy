...
struct Visitor : hilti::visitor::PreOrder {
    ...
    void operator()(operator_::function::Call* n) final {
        ...
        auto f = n->op0()->as<expression::Name>()->resolvedDeclaration()->as<declaration::Function>();
    ...
    void operator()(operator_::generic::New* n) final {
        if ( auto tv = n->op0()->type()->type()->tryAs<type::Type_>() ) {
    ...
    void operator()(operator_::string::Size* n) final { result = fmt("%s.size()", op0(n)); }
    ...
    cxx::Expression structMember(const expression::ResolvedOperator* o) {
        ...
        if ( auto f = op0->type()->type()->as<type::Struct>()->field(id); f->isOptional() ) {
            auto d = f->default_();
    ...
    void operator()(operator_::struct_::MemberCall* n) final {
        auto fdecl = dynamic_cast<const struct_::MemberCall&>(n->operator_()).declaration();
