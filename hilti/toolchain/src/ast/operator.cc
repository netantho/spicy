...
std::string _printOperator(operator_::Kind kind, const Operands& operands, const Meta& meta) {
    auto render_one = [](const OperandPtr& t) {
        if ( t->type__()->template isA<type::Member>() )
            return t->print();
        else if ( auto ft = t->type__()->tryAs<type::Function>(); ft && ft->functionNameForPrinting() )
    ...
    switch ( kind ) {
        case operator_::Kind::MemberCall: {
            assert(operands.size() == 3);
            std::string args;
            if ( auto ttype = operands[2]->type__()->tryAs<type::OperandList>() )
        ...
        case operator_::Kind::Call: {
            assert(operands.size() == 2);
            std::string args;
            if ( auto ttype = operands[1]->type__()->tryAs<type::OperandList>() )
                args = util::join(util::transform(ttype->operands(), [](const auto& x) { return x->print(); }), ", ");
            else
...
bool Operator::init(Builder* builder, const NodePtr& scope_root) {
    auto sig = Signature(signature(builder));
    ...
    if ( sig.result.type ) {
        ...
        _signature->result->type()->unify(builder->context(), builder->context()->root());
    }
    ...
    if ( _signature->result ) {
        ...
        if ( ! _signature->result->type()->unify(builder->context(), scope_root) )
            return false;
    }
...
std::string Operator::render() const {
    std::string x;
