...
hilti::struct_::MemberCall::MemberCall(const NodeDerivedPtr<declaration::Field>& fdecl)
...
QualifiedTypePtr _itemType(Builder* builder, const Expressions& operands) {
    if ( auto field =
             operands[0]->type()->type()->as<type::Struct>()->field(operands[1]->as<expression::Member>()->id()) )
        return field->type();
    else
...
void _checkName(expression::ResolvedOperator* op, bool check_optional = false) {
    ...
    auto f = op->op0()->type()->type()->as<type::Struct>()->field(id);
